...
//        return null;
    ...
    @Override
    public List<Country> findByContinentAndNameStartingWith(String continent, String name) throws Exception{
        List<Country> result = this.source.stream()
                .filter(country -> continent.equals(country.getContinent())&& country.getName().startsWith(name))
                .collect(Collectors.toList());
        if (result.isEmpty()){ result = this.source.stream()
                .filter(country -> continent.equals(country.getContinent())&& country.getCode().startsWith(name))
                .collect(Collectors.toList());
                if (result.isEmpty()){
    ...
    @Override
    public List<Country> findByContinentAndPopulationGreaterThanEqual(String continent, Integer population) throws Exception {
        if (population<0){
                    throw new Exception("Wrong Request");
                }
        
        List<Country> result = this.source.stream()
                .filter(country -> continent.equals(country.getContinent())&& country.getPopulation()>=population)
                .collect(Collectors.toList());
